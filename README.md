...
MarkDeck은 현재 **Electron + electron-vite 기반 desktop app** 입니다.
...
- Electron main -> preload -> renderer IPC 데이터 흐름
- React Router `HashRouter` 기반 desktop route state
- React Query 기반 desktop async state
- electron-vite `main / preload / renderer` 분리 빌드
...
### 2) Configure desktop environment (optional)
...
VITE_MARKDECK_APP_TITLE=MarkDeck
...
`electron-vite` will start the Electron app and the Vite renderer together.
...
| --- | --- | --- |
| `MARKDECK_CONTENT_ROOT` | No | Default content root for desktop runs. 앱 안에서 폴더를 직접 선택해 바꿀 수도 있습니다. |
| `VITE_MARKDECK_APP_TITLE` | No | App title shown in the renderer UI |
