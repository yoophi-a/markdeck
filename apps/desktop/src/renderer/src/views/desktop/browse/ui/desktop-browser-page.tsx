...
export function DesktopBrowserContent({ segments, initialEntries }: DesktopBrowserContentProps) {
  ...
  const rootLabel = getRootLabel(contentRootQuery.data);
  const heading = relativePath ? prettyPath(relativePath) : contentRootQuery.data ?? '/';
  ...
  return (
    <section className="stack">
      <div className="card">
        ...
        <h1>{heading}</h1>
        <Breadcrumbs rootLabel={rootLabel} segments={segments} />
      </div>
...
function getRootLabel(contentRoot?: string | null) {
  if (!contentRoot) {
    return '/';
  }
