...
const READER_LAYOUT_STORAGE_KEY = 'markdeck:reader-layout';
...
export function readReaderLayoutState(contentRootKey: string) {
  return readScopedState<ReaderLayoutState>(READER_LAYOUT_STORAGE_KEY, contentRootKey, [
    'showTree',
    'showFeedback',
    'showToc',
    'isDocumentMaximized',
...
export function writeReaderLayoutState(state: ReaderLayoutState) {
  writeState(READER_LAYOUT_STORAGE_KEY, state);
}
